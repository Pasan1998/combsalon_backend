...
//                $result = $db->query($sql);
                  ...
                  <div class="col-sm">
                    <input type="date" class="form-control" name="to" placeholder="Enter From Date"
                      max="<?php echo date('Y-m-d'); ?>" required=true>
              ...
              <table class="table table-striped jambo_table bulk_action">
                <thead>
                  <tr class="headings">
                    ...
                    <th class="column-title"> Total Sale </th>
                    <th class="column-title"> Total Salary Cost </th>
                    <th class="column-title"> Total Utility Cost </th>
                ...
                if (@$where == null) {
                  ...
                  $result1 = $db->query($query1);
                  // Perform the second query with date range
                  $query2 = "SELECT DATE(SaleDate) AS sale_day, Sum(SaleAmount) AS total_sale, 
                  SUM(TotalUtitlityCost) AS total_utitlity_cost, SUM(TotalSalaryCost) AS total_salary_cost, 
                  SUM(SaleCost) AS total_cost ,SUM(SaleProfit) AS total_profit ,SUM(SaleDiscount) AS total_discounts,
                  SUM(SalesSpecialdiscounts) AS total_special_discount FROM tbl_sales WHERE SaleDate BETWEEN '$from' AND '$to' GROUP BY DATE(SaleDate) ORDER BY DATE(SaleDate) DESC";
                  $result2 = $db->query($query2);
                  ...
                  while ($row2 = mysqli_fetch_assoc($result2)) {
                    ...
                    if (isset($combinedData[$date])) {
                      $combinedData[$date]['total_sale'] = $row2['total_sale'];
                      $combinedData[$date]['total_utitlity_cost'] = $row2['total_utitlity_cost'];
                      $combinedData[$date]['total_salary_cost'] = $row2['total_salary_cost'];
                      $combinedData[$date]['total_cost'] = $row2['total_cost'];
                      $combinedData[$date]['total_special_discount'] = $row2['total_special_discount'];
                      $combinedData[$date]['total_discounts'] = $row2['total_discounts'];
                      $combinedData[$date]['total_profit'] = $row2['total_profit'];
                      ...
                      $combinedData[$date] = array(
                        ...
                        'total_utitlity_cost' => $row2['total_utitlity_cost'],
                        'total_salary_cost' => $row2['total_salary_cost'],
                        'total_cost' => $row2['total_cost'],
                        'total_cashouts' => 0,
                        'net_total' => ($row2['total_cost'] + $row2['total_discounts'] + $row2['total_special_discount']),
                        'total_discounts' => $row2['total_discounts'],
                  ...
                  // Sort the datesArray in ascending order
                  sort($datesArray);

                  // Rebuild $netTotalArray based on the sorted $datesArray
                  $netTotalArray = array_map(function ($date) use ($combinedData) {
                    return $combinedData[$date]['finalprofitandloss'];
                  }, $datesArray);
                ...
                <tbody>
                  ...
                  foreach ($combinedData as $date => $data):
                    ...
                    <tr class="even pointer">
                      ...
                      echo number_format($total_total_sale, 2);
                      ?>

                      <td class=" "><?php
                      $total_total_utitlity_cost = $data['total_utitlity_cost'];
                      echo number_format($total_total_utitlity_cost, 2);
                      ?>

                      <td class=" "><?php
                      $total_total_salary_cost = $data['total_salary_cost'];
                      echo number_format($total_total_salary_cost, 2);
                      ?>

                      <td class=" "><?php
                      $total_cost_amount = $data['total_cost'];
                      echo number_format($total_cost_amount, 2);
                      ?>
                      ...
                      <?php @$total_total_sales += $data['total_sale'] ?> 
                      <?php @$total_total_utitlity_costs += $data['total_utitlity_cost'] ?>
                      <?php @$total_total_salary_costs += $data['total_salary_cost'] ?>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td>
                      <strong><?= number_format(@$total_total_sales, 2) ?></strong>
                    </td>

                    <td>
                      <strong><?= number_format(@$total_total_utitlity_costs, 2) ?></strong>
                    </td>

                    <td>
                      <strong><?= number_format(@$total_total_salary_costs, 2) ?></strong>
                    </td>
