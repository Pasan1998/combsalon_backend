...
//                $result = $db->query($sql);
                    ...
                    <div class="col-sm">
                    <input type="text" name="Employerid" value=<?= @$_POST['Employerid'] ?> >
                      <input type="text" name="Employerid" value=<?= @$_GET['EmpId'] ?> >
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
 ...
 echo $sql_date_range_sale_contribution=" SELECT date_range.date AS sale_day, COALESCE(SUM(s.SaleServiceSale), 0) AS total_sale
FROM (
...
WHERE s.EmplyoeeID = '$Employerid' AND DATE(s.SaleServiceAddedDate) BETWEEN '$from' AND '$to'
GROUP BY date_range.date
ORDER BY date_range.date DESC";

        $db = dbConn();
        $result_sale_contribution = $db->query($sql_date_range_sale_contribution);

        $data_labeldiscount = array();
        $data_valuediscount = array();

        while ($row_sale_contribution = $result_sale_contribution->fetch_assoc()) {
            $data_label_sale_contribution[] = $row_sale_contribution['total_sale'];
            $data_value_sale_contribution[] = $row_sale_contribution['sale_day'];
        }
       echo  "<br>";
        echo  "'" . implode("','", $data_value_sale_contribution) . "'";
        echo  implode(",", $data_label_sale_contribution);
...
<script>
  ...
  new Chart(ctx, {
    type: 'bar',
    data: {
      labels: [<?= "'" . implode("','", $data_label_sale_contribution) . "'" ?>],
      datasets: [{
        label: '# of Votes',
        data: [<?=  implode(",", $data_value_sale_contribution) ?>],
        borderWidth: 1,
